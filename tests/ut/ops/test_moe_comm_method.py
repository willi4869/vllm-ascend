...
class TestMoECommMethod(TestBase):
    ...
    def test_fused_mc2_comm_impl_uses_megamoe_path(self, mock_ctx):
        ...
        for is_decode_only in (False, True):
            for use_quant_method in (False, True):
                with self.subTest(is_decode_only=is_decode_only, use_quant_method=use_quant_method):
                    mock_ctx.is_decode_only_node = is_decode_only
                    quant_method = MagicMock() if use_quant_method else None
                    weights = (
                        quant_method.get_fused_mc2_weights.return_value if quant_method else fused_experts_input.weights
                    )
                    comm_impl._apply_cann_mega_moe.reset_mock()

                    result = comm_impl.fused_experts(fused_experts_input, quant_method=quant_method)

                    if quant_method:
                        quant_method.get_fused_mc2_weights.assert_called_once_with(fused_experts_input.layer)
                    comm_impl._apply_cann_mega_moe.assert_called_once_with(
                        fused_experts_input, weights, is_decode_only_node=is_decode_only
                    )
